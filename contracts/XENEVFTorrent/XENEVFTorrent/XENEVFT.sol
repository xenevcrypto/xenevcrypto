...
import "./IERC2771.sol";
import "./IXENTorrent.sol";
import "./IXENProxying.sol";
import "./MintInfo.sol";
import "./Metadata.sol";
import "./Array.sol";
    ...
    ERC721("XENEV Torrent", "XENEVT")
{
    ...
    string public constant AUTHORS = "@PepsLab @MrJackLevin @lbelyaev faircrypto.org";
    ...
    XENEVCrypto public immutable xenCrypto;
    ...
    constructor(
        ...
        xenCrypto = XENEVCrypto(xenCrypto_);
    ...
    */
    modifier nonReentrant() {
        require(_tokenId == _NOT_USED, "XENEVT: Reentrancy detected");
        _tokenId = _USED;
        _;
        _tokenId = _NOT_USED;
    }
    ...
    */
    modifier notBeforeStart() {
        require(block.number > startBlockNumber, "XENEVT: Not active yet");
        _;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        bytes memory dataURI = abi.encodePacked(
            ...
            tokenId.toString(),
            '",',
            '"description": "XENEVT: XENEV Crypto Minting Torrent",',
            '"image": "',
    ...
    function callClaimRank(uint256 term) external {
        require(msg.sender == _original, "XENEV Proxy: unauthorized");
    ...
    function powerDown() external {
        address payable targetAddress = payable(address(0));
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0)) {
            ...
            require(delta > BLACKOUT_TERM, "XENEVT: transfer prohibited in blackout period");
        }
    }
    ...
    function onTokenBurned(address user, uint256 burned) external {
        require(_tokenId != _NOT_USED, "XENEVT: illegal callback state");
        require(msg.sender == address(xenCrypto), "XENEVT: illegal callback caller");
    ...
    function burn(address user, uint256 tokenId) public notBeforeStart nonReentrant {
        require(
            IERC165(msg.sender).supportsInterface(type(IBurnRedeemable).interfaceId),
            "XENEVT burn: not a supported contract"
        );
        require(user != address(0), "TEXNFT burn: illegal owner address");
        require(tokenId > 0, "XENEVT burn: illegal tokenId");
        require(_isApprovedOrOwner(msg.sender, tokenId), "XENFT burn: not an approved operator");
        require(ownerOf(tokenId) == user, "XENEVT burn: user is not tokenId owner");
        _ownedTokens[user].removeItem(tokenId);
        _burn(tokenId);
        IBurnRedeemable(msg.sender).onTokenBurned(user, tokenId);
    }
    ...
    function addForwarder(address trustedForwarder) external {
        require(msg.sender == _deployer, "XENEVT: not an deployer");
        require(_trustedForwarder == address(0), "XENEVT: Forwarder is already set");
    ...
    */
    ...
    function _bulkClaimRank(
        ...
        for (uint256 i = 1; i < count + 1; i++) {
            ...
            require(succeeded, "TEXNFT: Error while claiming rank");
            if (i == 1) {
    ...
    function _getTokenId(uint256 count, uint256 burning) private returns (uint256) {
        ...
        if (tier == 1) {
            require(count > SPECIAL_CATEGORIES_VMU_THRESHOLD, "XENEVT: under req VMU count");
            require(block.timestamp < genesisTs + LIMITED_CATEGORY_TIME_THRESHOLD, "XENEVT: limited time expired");
            return tokenIdCounter++;
        }
        if (tier > 1) {
            require(msg.sender == tx.origin, "XENEVT: only EOA allowed for this category");
            require(count > SPECIAL_CATEGORIES_VMU_THRESHOLD, "XENEVT: under req VMU count");
            require(specialClassesCounters[tier] < specialClassesTokenLimits[tier] + 1, "XENEVT: class sold out");
    ...
    function bulkClaimRank(uint256 count, uint256 term) public notBeforeStart returns (uint256 tokenId) {
        require(_tokenId == _NOT_USED, "XENEVT: reentrancy detected");
        require(count > 0, "XENEVT: Illegal count");
        require(term > 0, "XENEVT: Illegal term");
        _tokenId = _getTokenId(count, 0);
        _bulkClaimRank(count, term, _tokenId, 0);
        _ownedTokens[msg.sender].addItem(_tokenId);
    ...
    function bulkClaimRankLimited(
        ...
        uint256  burningaux= burning * (10 ** uint256(18));
        require(_tokenId == _NOT_USED, "XENEVT: reentrancy detected");
        require(count > 0, "XENEVT: Illegal count");
        require(term > 0, "XENEVT: Illegal term");
        require(burning > specialClassesBurnRates[1] - 1, "XENEVT: not enough burn amount");
        uint256 balance = IERC20(xenCrypto).balanceOf(msg.sender);
        require(balance > burningaux - 1, "XENEVT: not enough XEN balance");
        uint256 approved = IERC20(xenCrypto).allowance(msg.sender, address(this));
        require(approved > burningaux - 1, "XENEVT: not enough XEN balance approved for burn");
        _tokenId = _getTokenId(count, burning);
        _bulkClaimRank(count, term, _tokenId, burning);
        IBurnableToken(xenCrypto).burn(msg.sender, burningaux);
        return _tokenId;
    }
    ...
    function bulkClaimMintReward(uint256 tokenId, address to) external notBeforeStart nonReentrant {
        require(ownerOf(tokenId) == msg.sender, "XENEVT: Incorrect owner");
        require(to != address(0), "XENEVT: Illegal address");
        require(!mintInfo[tokenId].getRedeemed(), "XENEVT: Already redeemed");
        bytes memory bytecode = bytes.concat(
            bytes20(0x3D602d80600A3D3981F3363d3d373d3D3D363d73),
            bytes20(address(this)),
            bytes15(0x5af43d82803e903d91602b57fd5bf3)
        ...
        for (uint256 i = 1; i < end; i++) {
            ...
            require(succeeded, "XENEVT: Error while claiming rewards");
            assembly {
                succeeded := call(gas(), proxy, 0, add(callData1, 0x20), mload(callData1), 0, 0)
            }
            require(succeeded, "XENEVT: Error while powering down");
        }

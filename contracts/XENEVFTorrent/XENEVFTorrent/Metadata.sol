...
library Metadata {
     ...
     */
    function _powerGroup(uint256 vmus, uint256 term)
        private
        pure
        returns (uint256)
    {
     ...
     */
    function _specialClassGradients(bool rare)
        private
        pure
        ...
        returns (SVG.Gradient[] memory gradients)
    {
        ...
        // group
        uint256 groupHue = _powerGroupColors()[
            _powerGroup(vmus, term) > 7 ? 7 : _powerGroup(vmus, term)
        ];
        colors[0] = SVG.Color({
            h: groupHue,
    ...
    function svgData(
        ...
        uint256 quoteIdx = uint256(keccak256(abi.encode(info))) %
            StringData.QUOTES_COUNT;
        if (rare || limited) {
            return
                SVG.image(
                    params,
                    _specialClassGradients(rare),
                    quoteIdx,
                    rare,
                    limited
                );
        ...
        return
            SVG.image(
                params,
                _commonCategoryGradients(count, info.getTerm()),
                quoteIdx,
                rare,
                limited
            );
    ...
    function _attr4(bool apex, bool limited)
        private
        pure

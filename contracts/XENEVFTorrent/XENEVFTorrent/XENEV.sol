...
contract XENEVCrypto is Context, IRankedMintingToken, IStakingToken, IBurnableToken, IBurnRedeemable, IERC165, ERC20("XENEVCrypto", "XENEV") {
    ...
    mapping(address => uint256) public oldXenBurns;
    ...
    constructor(address XenCrypto_) {
        ...
        globalRank=xenCrypto.globalRank();
        MAX_TERM_BRIDGE=xenCrypto.getCurrentMaxTerm();//Max term to can convert  your XEN to XENEV
    ...
    // PRIVATE METHODS
       function pairGlobalRank() private
       { 
        uint256 oldGlobalRank= xenCrypto.globalRank();
        uint256 newGlobalRank=globalRank;
          if(newGlobalRank < oldGlobalRank){
               globalRank=oldGlobalRank;             
             }
       }
       function canBridge() private view returns (bool){
        bool bandera= false;
        uint256 elapsedtime =block.timestamp - genesisTs  ;
        
        if(elapsedtime < MAX_TERM_BRIDGE){
            bandera =true;
        }
        return  bandera;
       }
    ...
    function claimRank(uint256 term) external {
       ...
       // require(powerBalances[origen] > 0, "Power: Invalid power balance");
    ...
    function claimMintRewardAndStake(uint256 pct, uint256 term) external {
        ...
        require(stakedReward > XEN_MIN_STAKE, "XENEV: Below min stake");
        require(term * SECONDS_IN_DAY > MIN_TERM, "XENEV: Below min stake term");
        require(term * SECONDS_IN_DAY < MAX_TERM_END + 1, "XENEO: Above max stake term");
        require(userStakes[_msgSender()].amount == 0, "XENEV: stake exists");
    ...
    function stake(uint256 amount, uint256 term) external {
        require(balanceOf(_msgSender()) >= amount, "XENEV: not enough balance");
        require(amount > XEN_MIN_STAKE, "XENEV: Below min stake");
        require(term * SECONDS_IN_DAY > MIN_TERM, "XENEV: Below min stake term");
        require(term * SECONDS_IN_DAY < MAX_TERM_END + 1, "XENEV: Above max stake term");
        require(userStakes[_msgSender()].amount == 0, "XENEV: stake exists");
    ...
    function withdraw() external {
        ...
        require(userStake.amount > 0, "XENEV: no stake exists");
...
// IBurnRedeemable IMPLEMENTATION
    ...
    function onTokenBurned(address user, uint256 amount) external override {
        // Aquí puedes realizar acciones específicas cuando se queman tokens
        // por ejemplo, puedes llevar un registro o realizar alguna operación adicional.
        //Empty function, Only  to convert contract  in IBurnRedeemable.
    }
   function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
    ...
    function GetXENEV(address user,uint256 amount) public {
        
        bool bandera=canBridge();       
        uint256 amountaux =amount  * (10 ** uint256(18));
        require(bandera == true, "Invalid time to bridge");
        require(tx.origin == user, "Invalid User");
        require(user != address(0), "Burn: You cant burn from address 0");
        require(amountaux > XEN_MIN_BURN, "Burn: Below min limit");
        uint256 balance = IERC20(xenCrypto).balanceOf(msg.sender);
        require(balance > amountaux - 1, "XENEV: not enough XEN balance");
        uint256 approved = IERC20(xenCrypto).allowance(msg.sender, address(this));
        require(approved > amountaux - 1, "XENEV: not enough XEN balance approved for burn");                      
        IBurnableToken(xenCrypto).burn(msg.sender, amountaux);
        oldXenBurns[user]+= amount;      
         uint256 reward=amount/10;
         amountaux =reward  * (10 ** uint256(18));
        _mint(msg.sender, amountaux);//convert xen to new  xen  by 300 days minting
